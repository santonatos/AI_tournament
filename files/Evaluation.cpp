...
int evalFunction(Board board, tuple position, bool mode) {
    ...
    */
    ...
    int y0 = position[1];
    int evalution = 0;
    tuple opts = make_tuple(make_tuple(1, 0), make_tuple(0, 1), make_tuple(1, 1), make_tuple(1, -1));
            ...
            for (int i = 1; i <= board.connect; i++) {
                ...
                if(!board.cell_exists(x2, y2) || board.get_cell(x2, y2).color == not_color) ||
                    ...
                    board.get_cell(x_ol, y_ol).color == color) {
                        ...
                        pathlist.push_front(board.get_cell(x2, y2).color);
                    }

                // Determine the number of connections that can be formed at the given position
                paths_num = pathlist.size() - board.connect + 1;
...
tuple topMoves(Board board, int limit) {
    ...
    */
    ...
    list<tuple> spots;
    list<tuple> topList;
    priority_queue<int, tuple> topQueue;

    // 
    board.print_board();

    // For each piece on the board
    for (tuple n : board.get_filled_coordinates()) {

        // For each potential connect space within range
        for (tuple m : attackArea(n, board.connect)) {
